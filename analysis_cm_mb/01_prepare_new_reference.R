# create a translation function between the old reference 
# and steve schaeffer's rearranged reference
...
# get the list of all possible informative sites
# (some these wont exist in the final data set)

gt_pos <- readRDS("meta/sampled_gt_seq_snps_all.rds") %>%
  select(CHROM, POS) %>%
  distinct
...
remap_df <- lapply(1:nrow(gt_pos), translate_positions)

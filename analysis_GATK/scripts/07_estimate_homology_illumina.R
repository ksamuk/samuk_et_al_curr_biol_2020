...
il_files <- list.files("data/allsites_recomb", full.names = TRUE)
vcf_file <- il_files[1]
...
parse_illumina_allsites <- function(vcf_file){
  ...
  vcf_df <- vcf_df %>%
    filter(gt_DP > 10, gt_RGQ > 80 | gt_GQ > 80) %>%
  ...
  data.frame(vcf_file = vcf_file, CHROM = interval_name[1], pos1 = interval_name[2], pos2 = interval_name[3], 
             diff_df)
    
}

il_homology <- mclapply(il_files, parse_illumina_allsites, mc.cores = 12)

il_homology %>%
  bind_rows %>%
  saveRDS(file = "data/illumina_homology.rds")
